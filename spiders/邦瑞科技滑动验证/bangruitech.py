...
class Toutiao:
    ...
    def get_sign(self):
        while True:
            time.sleep(2)
            html = self.driver.page_source
            img_base64 = re.search(r'image/png;base64,([\s\S]*?)&quot;\) \d', html)
            slider_base64 = re.findall(r'image/png;base64,([\s\S]*?)&quot;', html)
            if not slider_base64:
                continue
            slider_img = Image.open(BytesIO(base64.b64decode(slider_base64[-1].encode())))
            image = Image.open(BytesIO(base64.b64decode(img_base64.group(1).encode())))
            ...
            print(x, y)
            tracks = self.get_track(x + 20)
            self.dragging(tracks)
            time.sleep(2)
            html = self.driver.page_source
            if re.search(r'验证通过', html):
                time.sleep(1)
                print("验证通过")
    ...
    def dragging(self, tracks):
        button = self.driver.find_element_by_class_name('handler')
